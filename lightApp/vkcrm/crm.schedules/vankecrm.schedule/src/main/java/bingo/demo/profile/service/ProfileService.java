...
public class ProfileService extends BaseService {
	...
	public void activateProfile(String profileId) {
		// 非全局配置需要失效掉其他非全局配置
		SysProfile profile = rnDao.select(SysProfile.class, profileId);
	...
	public SysProfile getProfile(String profileId) {
		return rnDao.select(SysProfile.class, profileId);
	}
	...
	public SysProfileParameter getProfileParameter(String profileParameterId) {
		return rnDao.select(SysProfileParameter.class, profileParameterId);
	}
	...
	public boolean checkUniqueProfileName(String profileName, String profileId) {
		if (StringUtils.isEmpty(profileId)) {
			return rnDao.getJdbcDao().exists("profile.existProfileByName", profileName);
		} else {
			return rnDao.getJdbcDao().exists("profile.existProfileByNameForEdit", profileName, profileId);
		}
	}
	...
	public boolean checkUniqueProfileParameterName(String profileParameterName, String profileParameterId, String profileId) {
		if (StringUtils.isEmpty(profileId)) {
			return rnDao.getJdbcDao().exists("profile.existParameterByName", profileParameterName, profileId);
		} else {
			return rnDao.getJdbcDao().exists("profile.existParameterByNameForEdit", profileParameterName, profileParameterId ,profileId);
		}
	}
	...
	public void copyProfile(String sourceProfileId, String destinationProfileId) {
		...
		List<SysProfileParameter> sourceParams = rnDao.queryForList(SysProfileParameter.class, "profile.getProfileParams", queryParams);
