...
public class Creator4UserReport extends AbstractQuartzTask{
    private static final String SUCCESS_CODE = "0";//调用助这儿接口返回成功的标示
    private static final String SYNC_CODE = "creatReportUser";//写入同步日志表中的code;用于区别同步数据接口
	@Override
...
//        log.debug("*****************同步<Creator4UserReport任务>定时任务开始*****************");
//        if (new Date().getTime() < configSyncTime.getTime()) {
//            log.debug("[同步Creator4UserReport任务]-开始本次同步开始时间为："+configSyncTime+"，未到时间10秒后再试");
//            log.debug("*****************同步<Creator4UserReport任务>定时任务结束*****************");
...
//        appTaskService.callUserInfoReport_pro();
