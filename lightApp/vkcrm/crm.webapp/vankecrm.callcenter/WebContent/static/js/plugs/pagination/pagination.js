...
(function($) {
    ...
    function _tempFunc(data) {
        var arr = [];
        arr.push('<ul>');
        arr.push('<li data-index="-1" ' + (data.hasPrev ? 'class="disabled"' : '') + '><a>上一页</a></li>');
        for (var i = 0; i < data.left.length; i++) {
            var v = data.left[i];
            arr.push('<li data-index="' + v.index + '"><a>' + v.index + '</a></li>');
        };

        if (data.hasLeft) {
            arr.push('<li data-index="' + data.hasLeft + '"><a>...</a></li>');
        }
        for (var j = 0; j < data.center.length; j++) {
            var v = data.center[j];
            arr.push('<li data-index="' + v.index + '" ' + (v.isCurrent ? 'class="active"' : '') + '><a>' + v.index + '</a></li>');
        };
        if (data.hasRight) {
            arr.push('<li data-index="' + data.hasRight + '"><a>...</a></li>');
        }
        for (var k = 0; k < data.right.length; k++) {
            var v = data.right[k]
            arr.push('<li class="" data-index="' + v.index + '"><a>' + v.index + '</a></li>');
        };
        arr.push('<li data-index="+1" ' + (data.hasNext ? 'class="disabled"' : '') + '><a>下一页</a></li>');
        arr.push('</ul>');
        return arr.join("")
    }
