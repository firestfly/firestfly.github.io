...
import bingo.modules.securityConsole.common.ValidateUtil;
...
public class MessageService extends BaseService{
	...
	public ServiceResult sendMessage(String[] numbers,String content,String dstime,
			String extCode,String templateId,String type) throws Exception{
...
//		sb.append("&password=d368513a0c90f7b9c31535a2ad249592");
		...
		if(null != errorMessageMap.get(result)){
			...
			sendLog(newSendArray, content, timing, extCode, msgid, templateId, type, true,errorMessage);
			return ServiceResult.error(errorMessage);
		}else{
	...
	public void sendLog(String[] numbers,String content,Date timing,String extCode,
			String msgid,String templateId,String type,boolean success,String errorMessage){
		...
		ssl.setType(type);
		ssl.setSendName(SecurityContext.getCurrentUser().getName());
		ssl.setSendUid(SecurityContext.getCurrentUser().getId());
		ssl.setSendTime(new Date());
		// 保存发送日志
		callCenterDao.insert(ssl);
		...
		callCenterDao.batchInsert(SmsSendMobile.class, smsSendMobileList);
	}
