...
public class UserService extends BaseService {
	 ...
	 * @throws Exception 
	 */
	public ServiceResult saveOrUpdate(SecUser secUser) {
		String userId = secUser.getId();
		
		if(rnDao.exists("user.isMobilePhoneExist", secUser)){
		...
		return ServiceResult.succeed("保存用户信息成功！");
	}
	...
	public SecUser getUserById(String userId) {
		return rnDao.select(SecUser.class, userId);
	}

	// 根据UserId获取用户信息
	public Map<String, Object> getUserInfoById(String userId) {
		return rnDao.getJdbcDao().queryForMap("user.get.userinfo", userId);
	}

	// 检查登录帐号的唯一性
	public boolean checkUniqueAccount(String userId, String loginId) {
		if (StringUtils.isEmpty(userId)) {
			return rnDao.getJdbcDao().exists("user.checkAccount", loginId);
		} else {
			return rnDao.getJdbcDao().exists("user.checkAccountForUpdate", loginId, userId);
		}
	}
}
