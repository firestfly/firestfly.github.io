...
public class RoleService extends BaseService {
	...
	public SecRole getRoleById(String roleId) {
		return rnDao.select(SecRole.class, roleId);
	}
	...
	public void saveOrUpdate(SecRole secRole) {
		...
		if(StringUtils.isNotEmpty(secRole.getBelongtoSysId())){
			String[] belongtoSysArr = secRole.getBelongtoSysId().split(":");
			secRole.setBelongtoSysId(belongtoSysArr[0]);
			secRole.setBelongtoSysName(belongtoSysArr[1]);
		}
	...
	public boolean checkUniqueRoleName(String roleName, String roleId) {
		if (StringUtils.isEmpty(roleId)) {
			return rnDao.getJdbcDao().exists("role.existRoleByRoleName", roleName);
		} else {
			return rnDao.getJdbcDao().exists("role.existRoleByRoleNameForEdit", roleName, roleId);
		}
	}
	...
	public List<Map<String, Object>> getSelectedOperationDataRulePrivilegeList(String roleId,String entity) {
		if( null != entity &&"menu".equals(entity.trim())){
			return rnDao.getJdbcDao().queryForListMap("role.function.selectedRoleRuleList.menu", roleId);
		}
		return rnDao.getJdbcDao().queryForListMap("role.function.selectedRoleRuleList.operation", roleId);
	}

	public List<Map<String, Object>> getOperationDataRulePrivilegeList(String operation) {
		return rnDao.getJdbcDao().queryForListMap("role.function.selectedOperationRuleList", operation);
	}
