...
function assignRole(grid, rowData, keyData) {
	if (keyData) {
		var url = '~/modules/user/list_sec_user_role.jsp?belongtoSysId=1&userId=' + keyData;
		$.open(url, 680, 430);
	}
}
...
// 分配项目权限
function assignProject(grid, rowData, keyData) {
	if (keyData) {
		var roleName = rowData['Name'];
		var url = '~/modules/project/project_role.jsp?roleId=' + keyData + '&roleName=' + roleName;
		$.open(url, 900);
	}
}
