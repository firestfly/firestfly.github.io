$(function() {
			...
			$('#treeDiv').tree({// tree为容器ID
				...
				params : {
					...
					childSqlId : 'callcenter.organization.tree.children',
					parentId : 'root'
				}
			});
		});
...
function doOnLoaded() {
    if (document.getElementById("content_frame").src == "") {
        var url = Global.contextPath + '/modules/user/callcenter_list_user.jsp';
...
function loadUserContent() {
	document.getElementById("content_frame").src = Global.contextPath
			+ '/modules/user/callcenter_list_user.jsp';
}
...
function loadUserListByOrgId(orgId, name) {
	...
	document.getElementById("content_frame").src = Global.contextPath
			+ '/modules/user/callcenter_list_user.jsp?orgId=' + orgId + '&orgName=' + encodeURI(name);
}
