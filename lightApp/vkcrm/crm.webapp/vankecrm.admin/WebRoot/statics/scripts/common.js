...
function getContextPath() {
    ...
    //由于无法正常获取虚拟目录路径,所以判断更目录是否等于admin
    if (result === "/admin") {
        return result;
    }
    return "";
...
String.prototype.getQueryString = function (name) { //name 是URL的参数名字
    ...
    if (r = this.match(reg)) return (unescape(r[2]) || "").split("#")[0];
    return null;
};
...
$.uiwidget.register("ajaxlink", function (selector) {
    selector.live("click", function () {
        ...
        jsonOptions.before = jsonOptions.before || function () {
                return true;
            };
        jsonOptions.success = jsonOptions.success || function () {
                return true;
            };
        var data = {};
