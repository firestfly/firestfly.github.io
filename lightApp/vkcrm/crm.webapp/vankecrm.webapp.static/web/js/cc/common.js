(function(Common) {
    ...
    Common["ajax"] = function(opt) {
        ...
        if (opt.crossDomain && isIE(8)) {
            ...
            if (opt.type == 'POST' || opt.type == 'post') {
                return $.ajax({
                    url: opt.url,
                    data: opt.data,
                    contentType: opt.contentType ? opt.contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
                    type: 'POST',
                    dataType: 'json'
                }).done(function(data) {
                    opt.success && opt.success(data.details);
                });
        ...
        return jQuery.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                // Common["localdata"][this.url] = data;
                if (data.success) {
    ...
    Common["log"] = (function() {
        ...
        var hash = {
                ...
                "supplement": 14, //补录任务  创建来源source(传选中的文字内容)，处理方法processingWay（checkbox的文字内容）
                "supplementRecive": 15 //补录接听 参数 fullName  callTime  phoneNo
            }
            /* 通用参数为cTime(动作发生时间)
        ...
        return function(action, param) {
            ...
            return Common.ajax({
                url: servicePath.task + '/v1/callcenter/task/timeline/local',
                type: "POST",
                data: data,
                success: function() {},
                error: function() {},
