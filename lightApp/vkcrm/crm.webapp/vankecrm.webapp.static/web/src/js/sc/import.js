...
jQuery(document).ready(function() {
    ...
    $("#btnUploadNotPayCustomer").click(function() {
        ...
        if (fileUrl) {
            ...
            $("#btnUploadNotPayCustomer").attr("disabled", "disabled").addClass('disabled');
        } else {
            Common.tip.add({
                text: '上传文件不能为空',
                type: 'warning'
...
function downloadFileFrameOnload() {
    $("#btnUploadNotPayCustomer").removeAttr("disabled").removeClass('disabled');
    if (!ifFirst) {
        ...
        if (callbackResult.success) {
            ...
            $("#file_upload").val("");
            $("#frmUploadNotPayCustomer")[0].reset();
            document.getElementById("frmUploadNotPayCustomer").reset();
        } else {
            Common.tip.add({
                text: callbackResult.message,
                type: 'warning'
