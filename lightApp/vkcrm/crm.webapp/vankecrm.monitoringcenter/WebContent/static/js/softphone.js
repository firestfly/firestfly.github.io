window.SoftPhone = (function() {
    ...
    */
    ...
    var initObjectX = function(prop) {
        ...
        objectX.EnableWebServiceRequest = false; //是否连接cm service --------------------------
        objectX.WebServiceIPAddr = "10.11.15.20"; //129.1.202.3
        objectX.WebServicePort = 8000;
        objectX.WebServiceMethodName = "service/servlet/procM";

        //发送坐席状态信息至CM SERVICE
        objectX.WebServiceApiAgentStatusChg = "exec=true&methodName=agentStatus&callBack=0&execType=0&methodType=set&AgentID=[AgentId]&AgentStat=[AgentStation]&AgentType=[AgentType]&IPAddr=[IPAddr]&Status=[AgentCallStatus]&CallID=[CallId]&OtherDN=[OtherDN]&CallInType=[CallInType]&StateTime=[StateTime]";
        //软电话心跳
        ...
        objectX.TimeoutThresholdMap = "AS_Wrap=10;ReverseSpan=20;"; //"AS_Idle=10;ReverseSpan=10;";
    ...
    function trigger(eventName, valueStr) {
        if (window.console) {
            console.log(eventName + ':\t' + valueStr);
        }
        ...
        if (eventPool[eventName]) {
            var obj = valueStr ? format(valueStr) : null;
