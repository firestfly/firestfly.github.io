define(function(require, exports, module) {
    ...
    validator._required = function(caller, rule) { // VALIDATE BLANK FIELD
        ...
        if (callerType == "text" || callerType == "number" || callerType == "password" || tagName == "TEXTAREA" || callerType == "hidden") {
            if (!trim(caller.value)) {
                isError = true;
                promptText.push(rule.tips || "该输入项必填")
                return true;
            }
            ...
            if (checked === false) {
                ...
                if (checkbox.length == 1) {
                    promptText.push(rule.tips || "该选项为必选项")
                } else {
                    promptText.push(rule.tips || "必须选择一个选项")
                }
                return true;
            }
            ...
            if (!caller.value) {
                isError = true;
                promptText.push(rule.tips || "该选择项必选")
                return true;
            }
    ...
    validator._customRegex = function(caller, rule) { // VALIDATE REGEX RULES suport custom[email[errorInfo]] or email[errorInfo]
        ...
        if (!pattern.test(trim(caller.value))) {
            isError = true;
            promptText.push(rule.tips || validationRules[customRule]["alertText"]);
            return true;
        }
    }
    ...
    validator._length = function(caller, rule) { // VALIDATE LENGTH
        ...
        var minL = parseInt(mx[0]) || 0;
        var maxL = parseInt(mx[1]) || Infinity;
        ...
        if (feildLength < minL || feildLength > maxL) {
            isError = true;
            promptText.push(rule.tips || "当前输入长度为" + feildLength + " [ 输入长度必须在" + minL + (maxL > minL ? ("和" + maxL + "之间") : "以上") + " ]")
    ...
    validator._range = function(caller, rule) {
        ...
        if (callerType == "radio" || callerType == "checkbox") {
            ...
            if (groupSize < min || groupSize > max) {
                isError = true;
                promptText.push(rule.tips || ("必须选择" + min + (max > min ? ("到" + max) : '') + "选项"))
                return true;
            }
        } else {
            if (_isValueEmpty(caller)) {
            ...
            if (inputValue < min || inputValue > max) {
                isError = true;
                promptText.push(rule.tips || ("输入的值必须在" + min + "到" + max + "之间"))
                return true;
            }
        }
    }
    ...
    function loadValidation(caller) {
        ...
        for (var i = 0; i < getRules.length; i++) {
            getRules[i].replace(splitRegExp, function($0, $1, $2, $3) {
                // console.log($1, $2, $3)
                if ($1) {
                    name = $1;
                }
