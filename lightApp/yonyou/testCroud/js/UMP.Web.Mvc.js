;(function (root, factory) {
    ...
    if (typeof define === "function" && define.amd) {
        ...
        root.um = factory(root);
        root.$view = function (selector, createOptions) {
            return um.dom(selector, createOptions);
        };

        root.$model = function (selector) {
            selector = selector || '';
            return {
                get: function (childChain) {
                    return um.get(selector, childChain);
                },
                set: function (childChain, value) {
                    if (!childChain) return;
                    um.set(selector, value, {}, childChain);
                }
            }
        };
        root.$curModel = $model().get();
        root.$$ = function (arg) {
            var type = typeof arg;
            if (!arg) return;
            if (type === 'function') {
    ...
    EventEmitter.prototype.watch = function (selector, handler) {
        selector = _u.replaceBracket(selector);
        if (!this._watchers) {
            this._watchers = {};
        }
    ...
    EventEmitter.prototype.findWatcherDeps = function (selector) {
        ...
        // 查找父监控器
        // 示例: 比如选择器是"some.nested.value", 它也会触发监控"some"这个字段的监控器。
    ...
    /*
     * 替换字符串中的中括号
     * 例如 _u.replaceBracket("a[bc].c['123']")结果为a.bc.c.123
     * */
    _u.replaceBracket = function (selector) {
        if (_u.isString(selector)) {
            selector = selector.replace(/\[[\'|\"]*([\w|\u4E00-\u9FA5]+?)[\'|\"]*\]/img, function ($1, $2) {
                return '.' + $2;
            });
        }
    ...
    var expando = 'uQuery' + ('2.0' + Math.random()).replace(/\D/g, '');
    var globalCache = {};
    var uuid = 0;

    var setData = function (cache, name, value) {
        if (_u.isObject(cache) && _u.isString(name) && (value !== undefined)) {
            cache[name] = value;
        }
    };
    ...
    var setDataWithObject = function (cache, another) {
        uQuery.extend(true, cache, another);
    };
    ...
    var setAttr = function (element, attrObj) {
        if (_u.isObject(attrObj)) {
    ...
    //Data Cache Methods
    uQuery.prototype.data = function (name, value) {
        var element = this._element || this._elements[0];
        if (!element) return false;

        var cache = getCache(element);
        ...
        for (var i = 0, lenEl = elements.length; i < lenEl; i++) {
            ...
            for (var j = 0, lenEv = events.length; j < lenEv; j++) {
                if (element.addEventListener) {
                    ...
                    element.attachEvent('on' + events[j], fn);
                }
            }
        }
    }

    uQuery.prototype.bind = uQuery.prototype.on;

    uQuery.prototype.unbind = function (events, fn) {
        var self = this,
            elements = self._elements,
            i = 0,
            j = 0,
        ...
        events = events.split(" ");
        for (i = 0, lenEl = elements.length; i < lenEl; i++) {
            element = elements[i];
            for (j = 0, lenEv = events.length; j < lenEv; j++) {
                if (element.removeEventListener) {
                    element.removeEventListener(events[j], fn, false);
                } else if (element.detachEvent) {
                    element.detachEvent('on' + events[j], fn);
                }
            }
        }
    }
    ...
    uQuery.prototype.attr = function (attr, value) {
        var self = this,
            elements = self._elements;
        for (var i in elements) {
            if (value === undefined && _u.isObject(attr)) {
                setAttr(elements[i], attr);
            } else if (value === undefined && _u.isString(attr)) {
    ...
    uQuery.prototype.addClass = function (c) {
        ...
        if (c && _u.isString(c)) {
            ...
            for (var i in self._elements) {
                classList = self._elements[i].classList;
                for (i = 0; i < cLen; i++) {
                    c[i] && classList.add(c[i]);
                }
            }
        }
        return self;
    ...
    uQuery.prototype.hasClass = function (c) {
        var self = this;
        if (!c) return false;
        var element = this._element || this._elements[0];
    ...
    uQuery.prototype.toggleClass = function (c) {
        var self = this;
        for (var i in self._elements) self._elements[i].classList.toggle(c);
        return self;
    }
    ...
    //Extend Method
    uQuery.extend = uQuery.prototype.extend = function () {
        ...
        if (typeof target !== "object" && !_u.isFunction(target)) {
            target = {};
        }

        // extend uQuery itself if only one argument is passed
        if (length === i) {
            target = this;
            --i;
        }
        ...
        for (; i < length; i++) {
            ...
            if ((options = arguments[i]) != null) {
                ...
                for (name in options) {
                    ...
                    if (deep && copy && (_u.isObject(copy) || (copyIsArray = _u.isArray(copy)))) {
                        if (copyIsArray) {
                            copyIsArray = false;
                            clone = src && _u.isArray(src) ? src : [];

                        } else {
                            clone = src && _u.isObject(src) ? src : {};
                        }

                        // Never move original objects, clone them
                        target[name] = uQuery.extend(deep, clone, copy);
    ...
    UM.prototype.toJSON = function (options, element) {
        ...
        var self = this,
            element = element || self._element,
            data = self.dom(element).getDomValue(),
            options = options || self.dom(element).getOptions();
    ...
    UM.prototype.fromJSON = function (data, options, element) {
        ...
        self.dom(element).setDomValue(data, options);
    ...
    UM.prototype.getValue = function (element) {
        var self = this,
            element = element || self._element,
            selector = self.dom(element).getSelector();
        if (selector) {
            return um.get(selector);
        } else {
            return false;
        }
    ...
    UM.prototype.setValue = function (data, options, element) {
        var self = this,
            ...
            selector = self.dom(element).getSelector();
        if (selector) {
            um.set(selector, data, options);
        }
    }

    UM.prototype.getDomValue = function (element) {
    ...
    UM.prototype.setDomValue = function (data, options, element) {
        var self = this,
            element = element || self._element,
            options = options || self.dom(element).getOptions();
    ...
    UM.prototype.setDefault = function (force, options, element) {
        ...
        if (force) {
            ...
            self.dom(element).setDomValue(options.default, options);
        }
    ...
    UM.prototype.bind = UM.prototype.on;

    UM.prototype.unbind = function (events, fn, element) {
        var self = this,
            element = element || self._element;
        u.dom(element).unbind(events, fn);
        return this;
    };
    ...
    UM.prototype.updateBindings = function (selector) {
        ...
        bindings.forEach(function (element) {
            var focused = u.dom(element).get(0) === u.dom(":focus").get(0);
            if (!focused) {
    ...
    ////////////////////////////////////
    // DOM METHODS: GET - SET TABLES //
    ////////////////////////////////////

    /* UM.prototype.registerTables = function(element) {
     //初始化Tables
     var self = this;
     var selector = element || "table";
     var elements = u.dom(selector).get();
     var repeatSelector = "[" + tagPrefix + "-repeat]";

     for (var i in elements) {
     var table = elements[i];
     u.dom(table).find(repeatSelector)
    ...
    UM.prototype.registerRepeats = function (actionScope) {
        // 初始化repeats
        var self = this;
        ...
        if (actionScope) {
            elements = u.dom(actionScope).find(selector).get();
        } else {
            elements = u.dom(selector).get();
        }
        for (var i in elements) {
            var element = elements[i], options, repeatId;
            options = self.dom(element).getOptions();
            scope = self.dom(element).scope();
            repeatId = scope ? (scope + '.' + options.repeat) : (options.repeat);
            self._repeats[repeatId] = self._repeats[repeatId] || [];

            var wrapperAttr = tagPrefix + "-repeat-wrapper=\"" + self._repeatsCount + "\"",
                parent = u.dom(element).parent("[" + wrapperAttr + "]");
            if (!parent.length) {
                self._repeats[repeatId].push({
    ...
    /*UM.prototype.updateTables = function(selector){
    ...
    UM.prototype.getSelector = function (element) {
        var self = this,
            element = element || self._element,
            options = self.dom(element).getOptions(),
            scope = self.dom(element).scope(),
    ...
    UM.prototype.get = function (selector, childChain) {

        var self = this;
        if (childChain && _u.isString(childChain)) {
            selector = selector || '';
            selector += ('.' + childChain);
        }
    ...
    UM.prototype.set = function (selector, value, options, childChain) {
        if (childChain && _u.isString(childChain)) {
            selector = selector || '';
            selector += ('.' + childChain);
        }
    ...
    UM.prototype.remove = function (selector, options) {
        ...
        if (selector) {
            self.data = _json.undock(self.data, selector);
        } else {
            self.data = {};
        }
        self.updateDependencies(selector);
    ...
    var ajax = {
        send: function (data) {
            if (typeof (data) === 'object') {
                ...
                var xhr = root.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        ...
        jsonp: function (data) {
            ...
            var params = this.toQueryString(data.data);
            var oS = document.createElement('script');
            oHead.appendChild(oS);

            //创建jsonp回调函数
            root[callbackName] = function (json) {
            ...
            if (data.time) {
                oS.timer = setTimeout(function () {
                    ...
                    oHead.removeChild(oS);
                    data.fail && data.fail({message: "超时"});
                }, time);
            }
        },
    ...
    UM.prototype.ajax = function (data) {
        ajax.send(data);
    };
    ...
    var eventRemove = function (e) {
        e.preventDefault();
        var selector = um.dom(this).scope();
        var options = um.dom(this).getOptions();
        if (options && "remove" in options) {
            um.remove(selector, options);
        } else {
            return false;
        }
    }
    ...
    var eventDragStart = function (e) {
        /*   e.preventDefault();*/
        var touch = e.targetTouches[0];  // 把元素放在手指所在的位置
        var listenDirect = this.getAttribute(tagPrefix + '-drag') || 'horizontal';
        var listenDirectLowerLimit = +(this.getAttribute(tagPrefix + '-drag-l')) || 0;
        var listenDirectUpperLimit = +(this.getAttribute(tagPrefix + '-drag-u')) || 0;
        this.dragListener = {
            "direct": listenDirect,
            "direct_l": listenDirectLowerLimit,
            "direct_u": listenDirectUpperLimit
        };
        u.dom(this).data('touch', touch);
    }

    var eventDragDirection = function (xspace, yspace) {
    ...
    var eventDragCore = function (elem, dragSpace, isEnd) {
        var dragListener = elem.dragListener || {};
        var direct = dragListener.direct || 'h';
        var direct_l = dragListener.direct_l || 0;
        var direct_u = dragListener.direct_u || 0;
        var comparison = 0;
        var attrs = {'h': 'left', 'v': 'bottom'};
        var n;
        if (isEnd) {
            comparison = (direct_u - direct_l) / 2;
            dragSpace = 0;
        }

        if (direct == 'h' || direct == 'v') {
            n = (parseFloat(elem.style[attrs[direct]]) || 0);
            n = n + dragSpace;
            n <= (comparison || direct_l) && (n = direct_l);
            n > (comparison || direct_u) && (n = direct_u);
            elem.style[attrs[direct]] = n + 'px';
        }
    }

    var eventDragMove = function (e) {
        var originTouch = u.dom(this).data('touch');
        var touch = e.targetTouches[0];
        var dragSpace = {};
        dragSpace['h'] = touch.pageX - originTouch.pageX;
        dragSpace['v'] = touch.pageY - originTouch.pageY;

        if (this.moveDirection == undefined) {
            this.moveDirection = eventDragDirection(dragSpace['h'], dragSpace['v']);
        } else if (this.moveDirection == this.dragListener.direct) {
            eventDragCore(this, dragSpace[this.moveDirection]);
        }
    };

    var eventDragEnd = function (e) {
        eventDragCore(this, null, true);
        delete this.moveDirection;
    ...
    var setEventListeners = function () {
        ...
        u.dom("[" + tagPrefix + "-remove]").on("click", eventRemove);
        u.dom("[" + tagPrefix + "-push]").on("click", eventPush);
        u.dom("[" + tagPrefix + "-drag]").on("touchstart", eventDragStart);
        u.dom("[" + tagPrefix + "-drag]").on("touchmove", eventDragMove);
        u.dom("[" + tagPrefix + "-drag]").on("touchend", eventDragEnd);
    }
