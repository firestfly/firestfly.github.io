...
define(["avalon",
    ...
    var tempId = new Date() - 0, templateArr = template.split('MS_OPTION_EJS'), gridHeader = templateArr[0],
        ...
        remptyfn = /^function\s+\w*\s*\([^)]*\)\s*{\s*}$/m, sorting = false,
    ...
    var widget = avalon.ui.smartgrid = function (element, data, vmodels) {
        ...
        var vmodel = avalon.define(vmId, function (vm) {
            ...
            vm.sortColumn = function (column, index, event) {
                var target = event.target, $target = avalon(target), sortTrend = '', field = column.key, trend = 0,
            ...
            vm._getTemplate = function (defineDatas, startIndex) {
                var fn, html,
                    id = 'smartgrid_tmp_' + tempId, dt = defineDatas || vmodel.data, _columns = vmodel.columns,
                    ...
                    if (!callbacksNeedRemove.scrollCallback) {
                        callbacksNeedRemove.scrollCallback = avalon(window).bind('scroll', function () {
                            ...
                            var scrollTop = Math.max(document.body.scrollTop, document.documentElement.scrollTop),
                                offsetTop = $element.offset().top, headerHeight = avalon(smartgridHeader).css('height'),
                                top = scrollTop - offsetTop + vmodel.affixHeight,
                                clientHeight = avalon(window).height(), tableHeight = $element.outerHeight(),
    ...
    function bindEvents(options, containerWrapper) {
        ...
        if (type === 'Checkbox' || type === "Radio") {
            avalon.bind(containerWrapper, 'click', function (event) {
                var target = event.target, $target = avalon(target),
                    $row = avalon(target.parentNode.parentNode.parentNode), datas = options.data,
                    onSelectAll = options.onSelectAll, enabledData = options._enabledData,
                    disabledData = options._disabledData, dataIndex = $target.attr('data-index'),
    ...
    function perfectColumns(options, element, vmId) {
        ...
        for (var i = 0, len = columns.length; i < len; i++) {
            var column = columns[i], format = column.format, htmlFunction = '', _columnWidth = column.width,
