        ...
        .directive('pickadate', ['$locale', '$sce', '$compile', '$document', '$window', 'pickadateDateHelper',
            'pickadateI18n', 'pickadateModalBindings', 'filterFilter', function ($locale, $sce, $compile, $document, $window,
                ...
                var TEMPLATE =
                    ...
                    '<div class="pickadate-mask" ng-click="close()"></div>' +
                    '<div class="pickadate-container" ng-if="!ks">' +
                    '<div class="pickadate-header border-b">' +
                    '<div class="pickadate-controls">' +
                    '<a href="" class="pickadate-prev" ng-click="changeMonth(-1)" ng-class="{disabled: !allowPrevMonth}">' +
                    $sce.trustAsHtml(i18n.t('prev')) +
                    '</a>' +
                    '<a href="" class="pickadate-next" ng-click="changeMonth(1)" ng-class="{disabled: !allowNextMonth}">' +
                    ...
                    '<ul class="pickadate-cell pickadate-week">' +
                    '<li class="pickadate-head" ng-repeat="dayName in dayNames">' +
                    '{{dayName}}' +
                    '</li>' +
                    '</ul>' +
                    '<ul class="pickadate-cell pickadate-dates">' +
                ...
                return {
                    ...
                    link: function (scope, element, attrs, ngModel) {
                        ...
                        scope.changeMonth = function (offset) {
                            if (!offset) return;
                            if (offset == '1' && !this.allowNextMonth) return;
                            if (offset == '-1' && !this.allowPrevMonth) return;
                        ...
                        function render() {
                            ...
                            scope.dates = map(dates, function (date) {
                                ...
                                if (date.monthOffset != 0) {
                                    date.classNames = ['pickadate-hide'];
                                }
                                return date;
                            });
                        }
