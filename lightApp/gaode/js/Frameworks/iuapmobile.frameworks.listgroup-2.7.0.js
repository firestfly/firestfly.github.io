(function (root) {
    function ListGroup(selector, options) {
        ...
        me.$list = me.$element.children('ul.um-listgroup');
    ...
    ListGroup.prototype.init = function (options) {
        ...
        if (me.opts.collapsible) {
            me.$element.delegate('.um-listgroup-title', 'click', function () {
                var $elem = $(this);
        ...
        me.$list.on('touchstart', '.um-listgroup-row', function (e) {
        ...
        me.$list.on('touchmove', '.um-listgroup-row', function (e) {
            if (!me.config.itemSwipeEnable) return;
            ...
            switch (touchDirection) {
                case 'left':
                    ...
                    !me.config.hasOpeningItemMenu && me._events.trigger('itemSwipeLeft', me, $elem);
                    break;
                case 'right':
                    e.preventDefault();
                    e.stopPropagation();
                    me.config.isScroll = false;
                    !me.config.hasOpeningItemMenu && me._events.trigger('itemSwipeRight', me, $elem);
                    break;
                default:
                    me.config.isScroll = true;
            }
        ...
        me.$list.on('touchend', '.um-listgroup-row', function (e) {
        ...
        me.resizeFn = function () {
            if (me.opts.height > 0) {
                ...
                me.$parent.addClass('um-listgroup-adaptation');
            }
    ...
    ListGroup.prototype.itemClick = function (callback) {
        ...
        if (typeof callback == 'function') {
            ...
            if (!me.config.itemClickEnable) {
                me.$list.delegate('.um-listgroup-row', 'click', function (e) {
                    var $rows = me.$list.find('.um-listgroup-row');
                    var $target = $(this);
    ...
    ListGroup.prototype.itemDelete = function (callback) {
        ...
        if (typeof callback == 'function') {
            ...
            if (!me.config.itemDeleteEnable) {
                me.$list.delegate('.um-listgroup-row ' + '.um-delete', 'click', function (e) {
                    e.stopPropagation();
                    e.preventDefault();
    ...
    ListGroup.prototype.itemSwipeLeft = function (callback) {
        ...
        if (typeof callback == 'function') {
            ...
            me._events.on('itemSwipeLeft', function (sender, args) {
                var $target = args;
                var $rows = me.$list.find('.um-listgroup-row');
                var groupIndex = $target.closest('.um-listgroup-group').index();
                var childIndex = $target.index();
    ...
    ListGroup.prototype.itemSwipeRight = function (callback) {
        ...
        if (typeof callback == 'function') {
            me.config.itemSwipeEnable = true;
            me._events.on('itemSwipeRight', function (sender, args) {
                var $target = args;
                var $rows = me.$list.find('.um-listgroup-row');
                var groupIndex = $target.closest('.um-listgroup-group').index();
                var childIndex = $target.index();
    ...
    root.UM.ListGroup = ListGroup;
    root.UM.listgroup = function (selector, opts) {
