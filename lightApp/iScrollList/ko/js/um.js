...
$(function () {
    var PageTransitions = (function () {
        ...
        var $pages = $('div.um-page'),
            current = 0, // 初始化第一页
            endCurrPage = false,
            isCommonTrigger = 0, // 判断是否是浏览器后退和前进
            endNextPage = false,
            isReverse = 0,
        ...
        function init() {
            ...
            if ($(currentPage) && $(currentPage).length) {
                hashArr.push(currentPage);
            }
            $pages.eq(current).addClass('active');
        }

        // 处理hash数组
        function judgHistory(arr, s) {
            for (var j = arr.length, i = j - 1; i >= 0; i--) {
                if (arr[i] == s) {
                    // 如果hash数组有下一页的缓存，就触发返回，截取数组
                    arr.length = i + 2;
                    isReverse = 1;
                    break;
                } else {
        ...
        function enterPage(e) {
            ...
            judgHistory(hashArr, $nextPage);
            if ($(this).hasClass("um-back")) {
                isReverse = 1;
                hashArr.pop();
                $nextPage = $(hashArr[hashArr.length - 1]);
            } else if ($($nextPage).length) {
                hashArr.push($nextPage);
                $nextPage = $($nextPage);
                isReverse = 0;
            } else {
                return false;
            ...
            // 21 23 25 32 34 36 37 42 44 50,54,58(slide) 52(slide down)  56,60(slide up) 66(slidefade)

            var trans = $(this).data("transition") || 23;
            var options = {
                transition: trans,
                isReverse: isReverse,
                currPage: $currPage,
        ...
        window.onhashchange = function (d) {
            if (isCommonTrigger) return;
            var _currPage = "#" + d.oldURL.split("#")[1],
                _nextPage = "#" + d.newURL.split("#")[1];
            if (!$(_currPage).length && !$(_nextPage).length) return false;
            judgHistory(hashArr, _nextPage);
            var options = {
                transition: 50,
                isReverse: isReverse,
                currPage: $(_currPage),
        ...
        function setTransitionClass(flag) {
            switch (flag) {
                case "slide":
                    ...
                    inClass = 'active um-rotatePullRight';
                    if (isReverse) {
                        outClass = 'um-rotatePushRight';
                        inClass = 'active um-rotatePullLeft';
                    }
                    break;
                case "popup":
                    outClass = '';
                    inClass = 'active um-moveFromBottom';
                    if (isReverse) {
                        outClass = '';
                        inClass = 'active um-moveFromTop';
                    }
                    break;
                case 1:
                    outClass = 'um-moveToLeft';
                    inClass = 'active um-moveFromRight';

                    if (isReverse) {
                        outClass = 'um-moveToRight';
                        inClass = 'active um-moveFromLeft';
                    }
                    break;
                case 2:
                ...
                case 9:
                    ...
                    inClass = 'active um-moveFromRightFade';
                    if (isReverse) {
                        outClass = 'um-moveToRightFade';
                        inClass = 'active um-moveFromLeftFade';
                    }
                    break;
                case 11:
                    outClass = 'um-moveToTopFade';
                    inClass = 'active um-moveFromBottomFade';
                    if (isReverse) {
                        outClass = 'um-moveToBottomFade';
                        inClass = 'active um-moveFromTopFade';
                    }
                    break;
                case 13:
                ...
                case 21:
                    outClass = 'um-scaleDown';
                    inClass = 'active um-scaleUpDown um-delay300';
                    if (isReverse) {
                        outClass = 'um-scaleDownUp';
                        inClass = 'active um-scaleUp um-delay300';
                    }
                    break;
                case 23:
                    outClass = 'um-moveToLeft';
                    inClass = 'active um-scaleUp';
                    if (isReverse) {
                        outClass = 'um-moveToRight';
                        inClass = 'active um-scaleUp';
                    }
                ...
                case 27:
                    ...
                    inClass = 'active um-scaleUpCenter um-delay400';
                    break;
                case 28:
                    outClass = 'um-rotateRightSideFirst';
                    inClass = 'active um-moveFromRight um-delay200';
                    if (isReverse) {
                        outClass = 'um-rotateLeftSideFirst';
                        inClass = 'active um-moveFromLeft um-delay200';
                    }
                    break;
                case 30:
                    outClass = 'um-rotateTopSideFirst';
                    inClass = 'active um-moveFromTop um-delay200';
                    if (isReverse) {
                        outClass = 'um-rotateBottomSideFirst';
                        inClass = 'active um-moveFromBottom um-delay200';
                    }
                    break;
                case 32:
                    outClass = 'um-flipOutRight';
                    inClass = 'active um-flipInLeft um-delay500';
                    if (isReverse) {
                        outClass = 'um-flipOutLeft';
                        inClass = 'active um-flipInRight um-delay500';
                    }
                    break;
                case 34:
                    outClass = 'um-flipOutTop';
                    inClass = 'active um-flipInBottom um-delay500';
                    if (isReverse) {
                        outClass = 'um-flipOutBottom';
                        inClass = 'active um-flipInTop um-delay500';
                    }
                    break;
                case 36:
                ...
                case 37:
                    ...
                    inClass = 'active um-rotateInNewspaper um-delay500';
                    break;
                case 38:
                    outClass = 'um-rotatePushLeft';
                    inClass = 'active um-moveFromRight';
                ...
                case 42:
                    outClass = 'um-rotatePushLeft';
                    inClass = 'active um-rotatePullRight um-delay180';
                    if (isReverse) {
                        outClass = 'um-rotatePushRight';
                        inClass = 'active um-rotatePullLeft um-delay180';
                    }
                    break;
                case 44:
                    outClass = 'um-rotatePushTop';
                    inClass = 'active um-rotatePullBottom um-delay180';
                    if (isReverse) {
                        outClass = 'um-rotatePushBottom';
                        inClass = 'active um-rotatePullTop um-delay180';
                    }
                    break;
                case 46:
                ...
                case 50:
                    outClass = 'um-moveToLeftFade';
                    inClass = 'active um-rotateUnfoldRight';
                    if (isReverse) {
                        outClass = 'um-moveToRightFade';
                        inClass = 'active um-rotateUnfoldLeft';
                    }
                    break;
                case 52:
                    outClass = 'um-moveToBottomFade';
                    inClass = 'active um-rotateUnfoldTop';
                    if (isReverse) {
                        outClass = 'um-moveToTopFade';
                        inClass = 'active um-rotateUnfoldBottom';
                    }
                    break;
                case 54:
                    outClass = 'um-rotateRoomLeftOut';
                    inClass = 'active um-rotateRoomLeftIn';
                    if (isReverse) {
                        outClass = 'um-rotateRoomRightOut';
                        inClass = 'active um-rotateRoomRightIn';
                    }
                    break;
                case 56:
                    outClass = 'um-rotateRoomTopOut';
                    inClass = 'active um-rotateRoomTopIn';
                    if (isReverse) {
                        outClass = 'um-rotateRoomBottomOut';
                        inClass = 'active um-rotateRoomBottomIn';
                    }
                    break;
                case 58:
                    outClass = 'um-rotateCubeLeftOut';
                    inClass = 'active um-rotateCubeLeftIn';
                    if (isReverse) {
                        outClass = 'um-rotateCubeRightOut';
                        inClass = 'active um-rotateCubeRightIn';
                    }
                    break;
                case 60:
                    outClass = 'um-rotateCubeTopOut';
                    inClass = 'active um-rotateCubeTopIn';
                    if (isReverse) {
                        outClass = 'um-rotateCubeBottomOut';
                        inClass = 'active um-rotateCubeBottomIn';
                    }
                    break;
                case 62:
                    outClass = 'um-rotateCarouselLeftOut';
                    inClass = 'active um-rotateCarouselLeftIn';
                    if (isReverse) {
                        outClass = 'um-rotateCarouselRightOut';
                        inClass = 'active um-rotateCarouselRightIn';
                    }
                    break;
                case 64:
                    outClass = 'um-rotateCarouselTopOut';
                    inClass = 'active um-rotateCarouselTopIn';
                    if (isReverse) {
                        outClass = 'um-rotateCarouselBottomOut';
                        inClass = 'active um-rotateCarouselBottomIn';
                    }
                    break;
                case 66:
                    outClass = 'um-rotateSidesOut';
                    inClass = 'active um-rotateSidesIn um-delay200';
                    if (isReverse) {
                        outClass = 'um-rotateSlideOut';
                        inClass = 'active um-rotateSlideIn';
                    }
                    break;
